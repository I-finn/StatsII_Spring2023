...
#distribution. The p-value is calculated from the Kolmogorov- Smirno CDF:
...
print(ks_results)
1-.Call(stats:::C_pKolmogorov2x,  STATISTIC = ks_results$Dval, n = N)
# 6.106227e-15

#ks.test(data, "pnorm", alternative = "two.sided", exact = TRUE)
#ks.test(data, "pnorm", alternative = "two.sided", exact = FALSE)
k<-ks.test(data, "pnorm", alternative = "two.sided")
k$method
k
1-.Call(stats:::C_pKolmogorov2x,  STATISTIC = k$statistic, n = N)
...
s <- data ^ 2 * N
ps <- pmax(0, 1 - 2 * exp(-(2.000071 + .331 / sqrt(N) + 1.409 / N) * s))
plot(data, sapply(data, function (x) pks(x)), type='l')
lines(data, ps, lty=2)
#=----------------------------------------------

y1 = rnorm(n, mean = 0, sd = 1)
ks.test(datN, y1, alternative="less")
#kstest <-ks.test(empiricalCDF, pnorm)
...
plot(pnorm(data))
ggplot() +
  geom_point(aes(empiricalCDF, pnorm(data)))
...
set.seed(0)
datap <- rpois(n=20, lambda = 5)
ECDF <- ecdf(data)
empiricalCDFp <- ECDF(datap)
D <- max(abs(empiricalCDFp - pnorm(datap)))

ks.test(empiricalCDF, pnorm)
#psmirnov(empiricalCDF, c(20, 20))
