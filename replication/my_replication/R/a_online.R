...
# Replication - online experiment - spectators
rm(list=ls())
...
df_k <- read_csv(here("data","bl_online.csv"))  %>% 
...
#   ```{r}
df_k %>% ggplot(aes(x=y, y=1* (..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..])) + 
  geom_histogram() + facet_wrap(.~treatment_org) + theme_bw() + ylab("Fraction") + #,ncol=4
  xlab("Transfer from Lucky to Unlucky")
...
df_equal %>% 
  ...
  summarise(share_equal=mean(equal)) %>%
  knitr::kable(digits=3)
# ```
# ```{r}
ce <- df_equal %>%
  group_by(treatmentgroup) %>% summarise( yc = sum(y==4), n=n())
...
df_0 %>% 
  group_by(treatmentgroup) %>% 
  summarise(share_equal=mean(nothing)) %>%
  knitr::kable(digits=3)
# ```
# 
# ```{r}
c0 <- df_0 %>%
  group_by(treatmentgroup) %>% summarise( y0= sum(y==0), n=n())
...
nothingtw <- df_k %>%  group_by(treatmentgroup) %>% 
  summarise( nothing = sum(inequality == 1), n=n())
...
uz <- df_k %>% filter(treatment_org %in% c("Forced Choice (w)", 
  ...
  summarise(tw0 = sum(inequality==1),  n=n()) 
...
# ```
# 
output_stargazer(here("tables","online_1.tex"),w1, w2, w3, w4, w5, w6, 
          se = list(sqrt(diag(cluster.vcov(w1, cluster=1:nrow(df_k)))),
                    sqrt(diag(cluster.vcov(w2, cluster=1:nrow(df_k)))),
                    sqrt(diag(cluster.vcov(w3, cluster=1:nrow(df_k)))),
                    sqrt(diag(cluster.vcov(w4, cluster=1:nrow(df_k)))),
...
# ```
# 
output_stargazer(out=here("tables", "online_2.tex"), h1_1, h1_2, h1_3, h1_4, h1_5, h1_6, h1_7, h1_8,
           se = list( sqrt(diag(cluster.vcov(h1_1, cluster=1:length(h1_1$residuals)))),
                      sqrt(diag(cluster.vcov(h1_2, cluster=1:length(h1_2$residuals)))),
                      sqrt(diag(cluster.vcov(h1_3, cluster=1:length(h1_3$residuals)))),
                      sqrt(diag(cluster.vcov(h1_4, cluster=1:length(h1_4$residuals)))),
...
df_k %>% filter(!is.na(understanding2n)) %>% 
  ggplot(aes(x=understanding2n, y = (..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..])) + 
  geom_bar(width=0.7) + 
  facet_wrap(~treatment_org,ncol=4) + theme_bw() + 
  labs(x="Answer from 1-7. (1: no control, ..., 7: full control)", y = "Fraction") +
  scale_x_discrete(limits=c(1,2,3,4,5,6,7))
ggsave(here("graphs", "control_earnings.pdf"))
# ```
...
df_k %>% group_by(treatmentgroup) %>% 
  summarise(share_no_control = mean( (understanding2n==1), na.rm=TRUE),
            n = n()) %>% knitr::kable(digits=3)
# ```
# 
# Are these differences significant?
#   ```{r}
c1 <- df_k %>%  group_by(treatmentgroup) %>% summarise( no_control = sum(inequality == 1), n=n())
...
#```{r}
df_k %>% group_by(treatment) %>% summarise(share_dont_know = mean(understanding2=="Vet ikke")) %>%
  knitr::kable(digits=3)
#```
...
# sample on the subset of people who are absolutely sure.
#```{r}
